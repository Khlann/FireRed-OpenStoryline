...
## NEWS
- 🔄 **2026-06-21**：新增**多端会话同步**、**会话重命名/删除**以及可折叠的**设置**分组。
  - 服务端持久化最近会话列表与最后活跃会话，一处创建（如桌面）可在另一处（如手机）自动恢复。
  - 侧边栏每个会话支持重命名（✎）和删除（×）。
  - 模型/API 配置面板（LLM、VLM、Pexels、TTS、AI 转场）折叠到“设置”分组下。
  - 顶栏新增“会话”按钮，方便手机端快速打开会话列表。
